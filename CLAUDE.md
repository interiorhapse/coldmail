# ColdMail 프로젝트 컨텍스트

> **중요**: 새 세션 시작 시 이 파일을 반드시 먼저 읽어주세요.
> 마지막 업데이트: 2026-01-09
>
> **자동 업데이트**: 작업 완료 시 이 파일과 Git 커밋은 자동으로 반영됩니다.

---

## 1. 프로젝트 개요

### 1.1 서비스 설명
- **서비스명**: ColdMail - AI 기반 콜드메일 자동화 서비스
- **목적**: B2B 영업용 콜드메일 자동화 (기업 수집 → AI 분석 → 개인화 메일 생성 → 발송)
- **사용자**: GPTko AI사업팀 내부 사용

### 1.2 PRD 위치
```
c:\Users\jewel\Downloads\coldmail-prd.md
```

### 1.3 기술 스택
| 구분 | 기술 |
|------|------|
| Frontend | Next.js 14 (Pages Router), Tailwind CSS |
| Backend | Next.js API Routes |
| Database | Supabase (PostgreSQL) |
| AI | Google Gemini 3.0 Flash |
| Email | Resend |
| 크롤링 | Python 3.14 (requests, BeautifulSoup, Gemini HTTP API) |

---

## 2. 환경 설정

### 2.1 개발 서버
```bash
cd C:\Projects\coldmail
npm run dev -- -p 3001
```
- URL: http://localhost:3001

### 2.2 환경변수 (.env.local)
```
NEXT_PUBLIC_SUPABASE_URL=https://aekthtdeuqbxasahhnfp.supabase.co
NEXT_PUBLIC_SUPABASE_ANON_KEY=eyJ...
SUPABASE_SERVICE_ROLE_KEY=eyJ...
GEMINI_API_KEY=AIza...
RESEND_API_KEY=re_...
SENDER_EMAIL=jeweleg@gptko.co.kr
SENDER_NAME=GPTko AI사업팀
```

### 2.3 담당자 정보
| 이름 | 이메일 | 전화번호 | 역할 |
|------|--------|----------|------|
| 송제성 팀장 | jeweleg@gptko.co.kr | 010-7204-9650 | admin |
| 석준용 대리 | pillars3333@gptko.co.kr | - | user |

---

## 3. 핵심 규칙

### 3.1 "11" 규칙
사용자가 **"11"**이라고 하면:
1. 수정 후 **연관된 파일들 모두 체크**
2. **스텝바이스텝** 정확하게 작업
3. 시간 들여서 꼼꼼하게
4. 엮여 있는 것들 빠짐없이 수정

### 3.2 "12" 규칙
사용자가 **"12"**라고 하면:
1. **바이브 코딩 개발난이도 고려**
2. 복잡한 기능은 단순화 제안
3. 현실적인 구현 방안 우선

### 3.3 UX 원칙
- confirm/alert 대신 **Toast 알림** 사용 (자동 사라짐)
- 발송 같은 중요 작업은 **확인 팝업** 필수
- 선택 시 **하단 고정 액션바** 사용 (표 위에 액션바 X)
- 이모지 사용 금지 (이메일에서 엑박 됨)

### 3.4 Git 커밋 규칙
- **`.env.local` 절대 커밋 금지** (API 키 노출 위험)
- 환경변수는 로컬: `.env.local`, 배포: Vercel 대시보드에서 설정
- `.env.local.example`만 Git에 포함 (예시용)

---

## 4. 기능 현황

### 4.1 완료된 기능 ✅
- [x] 사용자 선택 로그인
- [x] 대시보드 (통계)
- [x] 기업 관리 (CRUD, 검색, 필터, 페이지네이션)
- [x] CSV 업로드
- [x] AI 기업 분석 (Gemini)
- [x] 템플릿 관리
- [x] 발송 대기열 관리 (추가, 수정, 조회, 삭제)
- [x] 발송 기능 (Resend 연동)
- [x] 발송 이력 조회
- [x] Toast 알림 시스템
- [x] 하단 고정 액션바 UX
- [x] 발송 확인 팝업
- [x] 기업 → 발송대기열 직행 (초안 스킵)
- [x] 발송 대기열 수정/조회 모달

### 4.2 해야 할 기능 🔲

#### Phase 1: 수집 모듈 (Python)
- [x] Python 크롤링 환경 세팅
- [x] 사람인 크롤러
- [ ] 로켓펀치 크롤러
- [ ] 원티드 크롤러
- [x] AI 파싱 (Gemini)
- [x] 크롤링 데이터 → Supabase 연동
- [x] 수집 설정 UI (소스 선택, 일일 한도)

#### Phase 2: 이메일 개선
- [ ] Resend 도메인 인증 (gptko.co.kr)
- [ ] 이메일 템플릿 HTML 디자인
- [ ] 로고 이미지 삽입 (`/images/logo.svg`)
- [ ] 발송 스케줄링

#### Phase 3: 고급 기능
- [ ] 이메일 오픈/클릭 추적
- [ ] 자동 후속 메일 (2차, 3차)
- [ ] A/B 테스트
- [ ] 리포트/분석

---

## 5. 폴더 구조

```
C:\Projects\coldmail\
├── public/
│   └── images/
│       └── logo.svg          # 기업 로고
├── scripts/
│   └── crawler/              # Python 크롤러
│       ├── main.py           # 메인 실행
│       ├── config.py         # 설정
│       ├── run_crawler.bat   # 자동 실행용
│       ├── sources/          # 수집 소스 (사람인, 로켓펀치, 원티드)
│       ├── parser/           # AI 파싱
│       └── storage/          # Supabase 연동
├── src/
│   ├── components/
│   │   ├── ui/               # 공통 UI 컴포넌트
│   │   │   ├── Button.jsx
│   │   │   ├── Modal.jsx
│   │   │   ├── Toast.jsx     # 토스트 알림
│   │   │   ├── ConfirmModal.jsx
│   │   │   ├── Table.jsx
│   │   │   ├── Badge.jsx
│   │   │   ├── Input.jsx
│   │   │   ├── Select.jsx
│   │   │   └── Tabs.jsx
│   │   ├── companies/        # 기업 관리 컴포넌트
│   │   │   ├── CompanyList.jsx
│   │   │   ├── CompanyForm.jsx
│   │   │   ├── CompanyDetail.jsx
│   │   │   └── CsvUpload.jsx
│   │   ├── send/             # 발송 관리 컴포넌트
│   │   │   ├── SendQueue.jsx
│   │   │   ├── SendHistory.jsx
│   │   │   ├── QueueEditModal.jsx  # 대기열 수정/조회
│   │   │   └── AiDraftModal.jsx
│   │   └── layout/
│   │       ├── Layout.jsx
│   │       ├── Header.jsx
│   │       └── Sidebar.jsx
│   ├── pages/
│   │   ├── _app.jsx          # ToastProvider 포함
│   │   ├── index.jsx         # 로그인
│   │   ├── dashboard.jsx
│   │   ├── companies/
│   │   │   └── index.jsx     # 기업 관리
│   │   ├── send/
│   │   │   └── index.jsx     # 발송 관리
│   │   ├── templates/
│   │   │   └── index.jsx
│   │   ├── settings/
│   │   │   └── index.jsx
│   │   └── api/
│   │       ├── auth/
│   │       │   ├── login.js
│   │       │   └── users.js
│   │       ├── companies/
│   │       │   ├── index.js      # GET/POST
│   │       │   ├── [id].js       # GET/PUT/DELETE
│   │       │   ├── industries.js
│   │       │   ├── analyze.js    # AI 분석
│   │       │   └── csv-upload.js
│   │       ├── send/
│   │       │   ├── queue.js          # 대기열 CRUD
│   │       │   ├── queue-direct.js   # 기업→대기열 직행
│   │       │   ├── send.js           # 실제 발송
│   │       │   └── history.js
│   │       ├── templates/
│   │       │   └── index.js
│   │       ├── drafts/
│   │       │   ├── index.js
│   │       │   └── generate.js   # AI 초안 생성
│   │       └── settings/
│   │           ├── index.js
│   │           └── demo-links.js
│   ├── lib/
│   │   ├── supabase.js       # Supabase 클라이언트 (지연 초기화)
│   │   ├── gemini.js         # Gemini AI (지연 초기화)
│   │   ├── resend.js         # Resend 이메일 (지연 초기화)
│   │   └── utils.js          # 유틸리티 함수
│   └── styles/
│       └── globals.css
├── .env.local                # 환경변수 (git 제외)
├── CLAUDE.md                 # 이 파일
└── package.json
```

---

## 6. 데이터베이스 (Supabase)

### 6.1 주요 테이블

| 테이블 | 설명 |
|--------|------|
| users | 사용자 |
| industries | 업종 (22개) |
| companies | 타겟 기업 |
| mail_templates | 이메일 템플릿 |
| drafts | AI 생성 초안 |
| send_queue | 발송 대기열 |
| mail_logs | 발송 이력 |
| signatures | 발신자 서명 |
| settings | 시스템 설정 |
| demo_links | 데모 링크 |

### 6.2 companies 테이블 주요 컬럼
- `name`, `industry_id`, `website`
- `contact_name`, `contact_email`, `contact_phone`
- `priority` (high/medium/low)
- `bm_summary`, `news_summary` (AI 분석 결과)
- `send_status` (미발송/1차완료/2차완료/3차완료)
- `send_count`, `last_send_date`

### 6.3 send_queue 테이블 주요 컬럼
- `company_id`, `to_email`
- `subject`, `body`
- `send_order` (1차/2차/3차)
- `status` (waiting/processing/complete/failed)
- `error_message`

---

## 7. API 엔드포인트

### 7.1 기업 관리
| Method | Endpoint | 설명 |
|--------|----------|------|
| GET | /api/companies | 목록 조회 |
| POST | /api/companies | 기업 추가 |
| GET | /api/companies/[id] | 상세 조회 |
| PUT | /api/companies/[id] | 수정 |
| DELETE | /api/companies/[id] | 삭제 |
| POST | /api/companies/analyze | AI 분석 |
| POST | /api/companies/csv-upload | CSV 업로드 |

### 7.2 발송 관리
| Method | Endpoint | 설명 |
|--------|----------|------|
| GET | /api/send/queue | 대기열 조회 |
| POST | /api/send/queue | 대기열 추가 (초안에서) |
| PUT | /api/send/queue | 대기열 수정 |
| DELETE | /api/send/queue?id=xxx | 대기열 삭제 |
| POST | /api/send/queue-direct | 기업→대기열 직행 |
| POST | /api/send/send | 실제 발송 |
| GET | /api/send/history | 발송 이력 |

---

## 8. 알려진 이슈

### 8.1 긴급
- **Resend 도메인 미인증**: gptko.co.kr 도메인 인증 필요
  - 확인: https://resend.com/domains
  - 인증 전까지 이메일 실제 발송 안 됨

### 8.2 개선 필요
- 이메일 HTML 템플릿 디자인 필요
- 로고 이미지를 이메일에 삽입하는 방법 구현 필요

---

## 9. 최근 변경 이력

### 2026-01-09
- **기업 관리 기능 개선**
  - 다중 삭제 기능 추가 (체크박스 선택 후 삭제)
  - CSV 다운로드 기능 추가 (선택/전체 기업 내보내기)
  - 하단 액션바에 삭제/CSV 다운로드 버튼 추가
- **크롤러 페이지 순환 로직 추가**
  - 매번 다음 페이지부터 크롤링 (중복 수집 방지)
  - 마지막 페이지 상태를 DB(settings)에 저장
  - 100페이지 넘으면 처음부터 순환
- **Python 크롤러 모듈 완성** (`scripts/crawler/`)
  - 사람인 크롤러 구현 및 데이터 품질 개선
  - Gemini AI 연락처 추출 (HTTP API 직접 호출)
  - Supabase 연동 (HTTP REST API)
  - Python 3.14 호환성 확보 (SDK 대신 HTTP API)
- 수집 설정 페이지 UI 개선
  - 수동 수집 버튼 추가
  - Toast 알림 연동
- 기업 목록 수집일 시간 표시 (`formatDateTime`)
- "12" 규칙 추가 (바이브 코딩 난이도 고려)

### 2026-01-08
- Toast 알림 시스템 구현
- 발송 대기열 수정/조회 모달 추가
- 발송 확인 팝업 추가
- 하단 고정 액션바 UX 개선
- 서명 이모지 제거 (엑박 문제)
- 기업 → 발송대기열 직행 기능
- 초안 목록 탭 제거 (불필요)
- UX 정렬 수정
- CLAUDE.md 문서 생성
- 로고 이미지 폴더 생성

---

## 10. 다음 세션 시작 시

```
1. 이 파일(CLAUDE.md) 읽기
2. 사용자의 요청 확인
3. 필요 시 관련 파일 읽기
4. 작업 진행
```

---

## 11. 크롤링 모듈 개발 가이드 (예정)

사용자가 크롤링 모듈 개발 요청 시 아래 프롬프트 참고:

```
Python 기반 이메일 수집 모듈 개발
- 타겟: 로켓펀치, 원티드, 구글 검색
- 수집 정보: 기업명, 담당자, 이메일, 웹사이트
- 기술: Selenium, BeautifulSoup, fake-useragent
- 출력: Supabase companies 테이블에 직접 INSERT
- 위치: scripts/email_crawler.py
```

---

## 12. 자주 쓰는 명령어

```bash
# 개발 서버 시작
cd C:\Projects\coldmail && npm run dev -- -p 3001

# 빌드
npm run build

# Supabase SQL 에디터
# https://supabase.com/dashboard/project/aekthtdeuqbxasahhnfp/sql
```
